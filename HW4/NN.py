...
class NeuralNet:
    ...
    def silu(self, u):
        ...
        """
        sigmoid_u = 1 / (1 + np.exp(-u)) # compute sigmoid
        return u * sigmoid_u # multiply element-wise
    ...
    def derivative_silu(self, x):
        ...
        return sigmoid_x + x * sigmoid_x * (1 - sigmoid_x) # derivative 
    ...
    def softmax(self, u):
        ...
        """
        u_max = np.max(u, axis=1, keepdims=True)  # determine max for each row
        u_exp = np.exp(u - u_max)  # take exponent of shifted inputs
        sum_u_exp = np.sum(u_exp, axis=1, keepdims=True)  # sum exponents
        return u_exp / sum_u_exp  # normalization
    ...
    def cross_entropy_loss(self, y, y_hat):
        ...
        y_hat_clipped = y_hat + 1e-15  # avoid log(0)
        return -np.sum(y * np.log(y_hat_clipped)) / N  # compute avg loss
    ...
    def _dropout(u, prob):
        ...
        """
        dropout_mask = (np.random.rand(*u.shape) > prob).astype(float) # generate mask 
        u_after_dropout = u * dropout_mask # apply mask 
        u_after_dropout /= (1 - prob) # scale
    ...
    def forward(self, x, use_dropout):
        ...
        """
        self.cache["X"] = x
        u1 = x @ self.parameters["theta1"] + self.parameters["b1"]  # compute pre-activation
        o1 = self.silu(u1)  # apply SiLU activation

        if use_dropout:
            o1, dropout_mask = self._dropout(o1, self.dropout_prob) # dropout mask
            self.cache["mask"] = dropout_mask  # store dropout mask
        else:
            self.cache["mask"] = np.ones_like(o1) 
        self.cache["u1"], self.cache["o1"] = u1, o1  # cache for backpropagation
    ...
    def compute_gradients(self, y, yh, use_dropout):
        ...
        # get cached values
        o2 = self.cache["o2"]    # (N, R)
        u2 = self.cache["u2"]    # (N, R)
        o1 = self.cache["o1"]    # (N, K)
        u1 = self.cache["u1"]    # (N, K)
        x = self.cache["X"]      # (N, M)
        dropout_mask = self.cache["mask"]  # (N, K)

        # gradients for theta3 and b3
        dLoss_theta3 = o2.T @ dLoss_u3  # (R, D)
        dLoss_b3 = np.sum(dLoss_u3, axis=0)  # (D,)

        # back prop  to second hidden layer
        dLoss_o2 = dLoss_u3 @ self.parameters["theta3"].T  # (N, R)
        dActivation_u2 = self.derivative_silu(u2)          # (N, R)
        dLoss_u2 = dLoss_o2 * dActivation_u2              # element-wise multiplication

        # gradient for theta2 and b2
        dLoss_theta2 = o1.T @ dLoss_u2  # (K, R)
        dLoss_b2 = np.sum(dLoss_u2, axis=0)  # (R,)

        # back prop to first hidden layer
        dLoss_o1 = dLoss_u2 @ self.parameters["theta2"].T  # (N, K)
        dActivation_u1 = self.derivative_silu(u1)          # (N, K)

        if use_dropout:
            # apply mask
            scaling_factor = 1 / (1 - self.dropout_prob)
            dLoss_u1 = dLoss_o1 * dActivation_u1 * dropout_mask * scaling_factor
        else:
            dLoss_u1 = dLoss_o1 * dActivation_u1 

        # gradient for theta1 and b1
        dLoss_theta1 = x.T @ dLoss_u1  # (M, K)
        dLoss_b1 = np.sum(dLoss_u1, axis=0)  # (K,)
    ...
    def update_weights(self, dLoss, use_momentum):
        ...
        if use_momentum:
            for p in self.parameters:
                self.change[p] = self.momentum * self.change[p] + dLoss[p] # update momentum
                self.parameters[p] -= self.learning_rate * self.change[p] # update params
        else:
            for p in self.parameters:
                self.parameters[p] -= self.learning_rate * dLoss[p] # gradient descent update
        self.weight_updates += 1  # count++
    ...
    def backward(self, y, yh, use_dropout, use_momentum):
        ...
        """
        gradients = self.compute_gradients(y, yh, use_dropout) # compute gradients
        self.update_weights(gradients, use_momentum) # update weights
        return (
            gradients["theta3"], gradients["b3"],
            gradients["theta2"], gradients["b2"],
            gradients["theta1"], gradients["b1"]
        )
    ...
    def gradient_descent(self, x, y, iter=60000, use_momentum=False, local_test=False):
        ...
        **For LOCAL TEST append and print out loss at every iteration instead of every 1000th multiple.
        ...
        self.init_parameters()
        for i in range(iter):
            y_hat = self.forward(x, self.use_dropout) # forward pass
            loss = self.cross_entropy_loss(y, y_hat) # compute loss
            self.backward(y, y_hat, self.use_dropout, use_momentum) # compute weights backwards
            print_multiple = 1 if local_test else 1000 # print stuffs
            if i % print_multiple == 0:
                print("Loss after iteration %i: %f" % (i, loss))
                self.loss.append(loss)
    ...
    def batch_gradient_descent(self, x, y, use_momentum, iter=60000, local_test=False):
        ...
        for i in range(iter):
            # find start and end
            start_idx = (self.iteration * self.batch_size) % N
            ...
            if end_idx <= N:
                ...
                # batch wraps
                end_idx_mod = end_idx % N
                x_batch = np.concatenate((x[start_idx:], x[:end_idx_mod]), axis=0)
                y_batch = np.concatenate((y[start_idx:], y[:end_idx_mod]), axis=0)

            y_hat = self.forward(x_batch, self.use_dropout) # forward pass
            loss = self.cross_entropy_loss(y_batch, y_hat) # compute loss
            self.backward(y_batch, y_hat, self.use_dropout, use_momentum) # update weights backwards
            self.iteration += 1 # count++
            print_multiple = 1 if local_test else 1000 # printing stuffs
            if i % print_multiple == 0:
                print("Loss after iteration %i: %f" % (i, loss))
                self.loss.append(loss)
                self.batch_y.append(y_batch)
        self.iteration = 0 # reset count
