...
class RNN(BaseSequentialModel):
    ...
    def define_model(self):
        ...
        # embedding layer
        self.model.add(tf.keras.layers.Embedding(input_dim=self.vocab_size, output_dim=embedding_dim))

        # simpleRNN layer
        self.model.add(tf.keras.layers.SimpleRNN(units=rnn_units))

        # dense layer
        self.model.add(tf.keras.layers.Dense(units=self.vocab_size))
